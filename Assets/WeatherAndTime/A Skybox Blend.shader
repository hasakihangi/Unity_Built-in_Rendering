...
Shader "A/ProceduralSkybox_BlendTexture" {
Properties {
    ...
    // cloudy texture
	[NoScaleOffset] _Texture ("Cloudy Texture (HDR)", Cube) = "grey" {}
	_TextureTint("Cloudy Texture Tint", Color) = (1,1,1,1)
	_TextureExposure("Cloudy Texture Exposure", Range(0,8)) = 1.0
    _Blend("Cloudy Blend Factor", Range(0,1)) = 0.0

	// sunny texture
    [NoScaleOffset] _SunnyTexture ("Sunny Texture (HDR)", Cube) = "grey" {}
    _SunnyTextureTint("Sunny Texture Tint", color) = (1,1,1,1)
    _SunnyTextureExposure("Sunny Texture Exposure", Range(0,6)) = 1.0
    _SunnyBlend("Sunny Blend Factor", Range(0,1)) = 0.0

	// Lightning
    _LightningStrength("Lightning Color Strength", Range(0,1)) = 0.0
	_LightingRange("Lightning Range From Top", Range(0,1)) = 0.0
}
...
SubShader {
    ...
    Pass {
        ...
        half4 _Texture_HDR;
        half4 _TextureTint;
        half _TextureExposure;
        float _TextureRotation;
        float _Blend;

        // sunny
        samplerCUBE _SunnyTexture;
        float4 _SunnyTexture_HDR;
        half4 _SunnyTextureTint;
        half _SunnyTextureExposure;
        float _SunnyTextureRotation;
        float _SunnyBlend;
        
        // Lightning
        half _LightningStrength;
		float _LightingRange;
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
            ...
            //
            //float3 rotated = RotateAroundYInDegrees(v.vertex, _TextureRotation);
            // OUT.
        ...
        // if y < 0 [eyeRay.y > 0] - sky
            ...
            IN.texcoord = RotateAroundYInDegrees(IN.texcoord, _TextureRotation);
            
			// rainy
            float4 tex = texCUBE(_Texture, IN.texcoord);
            float3 c = DecodeHDR(tex, _Texture_HDR);
            c = c * _TextureTint.rgb * unity_ColorSpaceDouble.rgb;
            c *= _TextureExposure;

            // sunny
            tex = texCUBE(_SunnyTexture, IN.texcoord);
            float3 c2 = DecodeHDR(tex, _SunnyTexture_HDR);
            c2 = c2 * _SunnyTextureTint.rgb * unity_ColorSpaceDouble.rgb;
            c2 *= _SunnyTextureExposure;
            
            // Lerp col and c and c2 -> color
            float3 color;
            
            float rainyWeight = _Blend;
            float sunnyWeight = (_Blend + _SunnyBlend > 1.0) ? 1.0-_Blend : _SunnyBlend;
            float proceduralWeight = 1.0 - rainyWeight - sunnyWeight;

            color = rainyWeight * c + sunnyWeight * c2 + proceduralWeight * col;

            color += half3(_LightningStrength, _LightningStrength, _LightningStrength);
