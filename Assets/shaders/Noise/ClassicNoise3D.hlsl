...
// AO MODIFICATION
void Remap_noise(float In, float2 InMinMax, float2 OutMinMax, out float Out)
{
    Out = OutMinMax.x + (In - InMinMax.x) * (OutMinMax.y - OutMinMax.x) / (InMinMax.y - InMinMax.x);
}

float PerlinNoise3D(float3 sampleCoordinates, float scale)
{
    float3 coordinates = sampleCoordinates * scale;
    float output;
    PerlinNoise3D_float(coordinates, output);
    Remap_noise(output, float2(-1.15, 1.15), float2(0,1), output);
    return output;
}
// END AO MODIFICATION
